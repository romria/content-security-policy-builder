...
export type CustomIdKey = 'floodID' | 'ctmID' | 'zendeskSubdomain';

export type DynamicValue = {
  var: CustomIdKey;
  getVal: (v: string) => string;
};

export type CspValue = string | DynamicValue;

export type FeatureConfig = Partial<Record<CspDirective, CspValue[]>>;

export const CSP_KEYS: CspDirective[] = [
  'script-src', 'script-src-elem', 'script-src-attr',
  'style-src', 'style-src-elem', 'style-src-attr',
  'img-src', 'font-src', 'manifest-src', 'connect-src',
  'object-src', 'media-src', 'worker-src',
...
const dv = (varKey: CustomIdKey, fn: (v: string) => string): DynamicValue => ({
  var: varKey,
  getVal: fn,
});

// Shared across gTLD img-src and connect-src to avoid duplication
const GOOGLE_TLDS: string[] = [
...
export const FEATURES = {
  ...
  ga4signal: { // https://developers.google.com/tag-platform/tag-manager/web/csp#google_analytics_4_google_analytics (Google Signals)
    ...
    'connect-src': ['https://*.google-analytics.com', 'https://*.analytics.google.com', 'https://*.googletagmanager.com', 'https://*.g.doubleclick.net', 'https://*.google.com', 'https://pagead2.googlesyndication.com'],
    'frame-src': ['https://www.googletagmanager.com'],
  },
  gTLD: {
    'img-src': GOOGLE_TLDS,
    'connect-src': GOOGLE_TLDS,
  },
  gAds: {
  ...
  gRecaptcha: { // https://developers.google.com/recaptcha/docs/faq#im-using-content-security-policy-csp-on-my-website.-how-can-i-configure-it-to-work-with-recaptcha
    'script-src': ['https://www.google.com/recaptcha/', 'https://www.gstatic.com/recaptcha/'],
    'frame-src': ['https://www.google.com/recaptcha/', 'https://recaptcha.google.com/recaptcha/'],
    'connect-src': ['https://www.google.com/recaptcha/'],
  },
  floodl: { // https://developers.google.com/tag-platform/tag-manager/web/csp#floodlight
    'img-src': [dv('floodID', v => `https://${v}.fls.doubleclick.net`), 'https://adservice.google.com', 'https://www.googletagmanager.com'],
  ...
  floodCS: {
    'frame-src': [dv('floodID', v => `https://${v}.fls.doubleclick.net`), 'https://www.googletagmanager.com'],
  },
  floodImg: {
  ...
  zendesk: { // https://developer.zendesk.com/documentation/zendesk-web-widget-sdks/sdks/web/csp/
    'script-src': ['https://static.zdassets.com', 'https://ekr.zdassets.com', 'https://ekr.zendesk.com', dv('zendeskSubdomain', v => `https://${v}.zendesk.com`), 'https://*.zopim.com', 'https://zendesk-eu.my.sentry.io'],
    'connect-src': ['https://static.zdassets.com', 'https://ekr.zdassets.com', 'https://ekr.zendesk.com', dv('zendeskSubdomain', v => `https://${v}.zendesk.com`), 'https://*.zopim.com', 'https://zendesk-eu.my.sentry.io', dv('zendeskSubdomain', v => `wss://${v}.zendesk.com`), 'wss://*.zopim.com'],
    'style-src': ["'unsafe-inline'"],
  ...
  ctm: {
    'script-src': [dv('ctmID', v => `https://${v}.tctm.co`)],
    'connect-src': [dv('ctmID', v => `https://${v}.tctm.co`)],
  },
  ...
  sentry: { // https://docs.sentry.io/platforms/javascript/install/cdn/
